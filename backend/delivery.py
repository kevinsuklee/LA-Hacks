...
def purchase_options(merchant_id):
	payload = {'order_type': 'delivery'}
	payload['order_time': #some time stamp]
	new_url = 'https://api.delivery.com/customer/cart/' + merchant_id + '/checkout'
	global delivery
	return delivery.get(new_url, params = payload)

def complete_purchase(merchant_id, payment_method = 'cash',tip = 1.00, location_id):
	payload = {'tip': tip, 'location_id': location_id, 'payments': [{'type': payment_method}], "order_type": "delivery"}
	payload['order_time': #some time stamp]
	new_url = 'https://api.delivery.com/customer/cart/' + merchant_id + '/checkout'
	global delivery
	return delivery.get(new_url, params = payload) 
